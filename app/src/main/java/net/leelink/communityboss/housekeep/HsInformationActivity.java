...
//                                tv_close_time.setText(dateTime_close);
                                Glide.with(HsInformationActivity.this).load(Urls.getInstance().IMG_URL + json.getString("regist_path")).into(img_store_head);
                                Glide.with(HsInformationActivity.this).load(Urls.getInstance().IMG_URL + json.getString("store_font_path")).into(img_publicity);
                                Glide.with(HsInformationActivity.this).load(Urls.getInstance().IMG_URL + json.getString("license_path")).into(img_license);


                            } else {
                                Toast.makeText(HsInformationActivity.this, json.getString("message"), Toast.LENGTH_LONG).show();
                            }
...
//                type = 3;
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        try {
                            ...
                            if (json.getInt("status") == 200) {
                                Toast.makeText(HsInformationActivity.this, "信息提交成功", Toast.LENGTH_SHORT).show();
                                finish();
                            } else {
                                Toast.makeText(HsInformationActivity.this, json.getString("message"), Toast.LENGTH_LONG).show();
                            }
                        ...
                        public void onSuccess(Response<String> response) {
                            try {
                                ...
                                if (json.getInt("ResultCode") == 200) {
                                    Toast.makeText(HsInformationActivity.this, "信息已提交审核,请重新登录", Toast.LENGTH_SHORT).show();
                                    SharedPreferences sp = getSharedPreferences("sp", 0);
                                    SharedPreferences.Editor editor = sp.edit();
                                    editor.remove("AppToken");
                                    editor.apply();
                                    Intent intent = new Intent(HsInformationActivity.this, LoginActivity.class);
                                    intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                                    startActivity(intent);
                                    finish();
                                } else {
                                    Toast.makeText(HsInformationActivity.this, json.getString("ResultValue"), Toast.LENGTH_LONG).show();
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
        ...
        if (data != null) {
            switch (requestCode) {
                case 1:     //相册
                    ...
                    bitmap = BitmapCompress.decodeUriBitmap(HsInformationActivity.this, uri);
                    switch (type) {
                        case 0:
    ...
    private void popu_head() {
        ...
        popview = LayoutInflater.from(HsInformationActivity.this).inflate(R.layout.popu_head, null);
        ...
        popuPhoneW.setOnDismissListener(new HsInformationActivity.poponDismissListener());
    }
